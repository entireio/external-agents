...
- The authoritative transcript is written by the plugin during the `session.status` (turn-end) event. `prepare-transcript` can refresh it on demand via `kilo session show --format json` (see `command_runner.go`); mid-turn reads otherwise see whatever was last written at turn-end.
